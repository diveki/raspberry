...
def get_motor_status(m):
	# visszaadott ertek , hogy a motor forog e vagy sem, azaz a motor is_active attributuma (ami vagy True vagy False)
...
def main():
    ...
    try:
        ...
        while True:
            if data_timer <= 0:
                tt = # merd a homersekletet a get_temp fuggveny hasznalataval
                hum = # merd a paratartalmat a get_hum fuggveny hasznalataval
                # hivd meg a ventillation fuggvenyt, ami eldonti, hogy be kell e kapcsolni a motort vagy sem
                device.publish_data()
